functions {
  // [OPTIMIZATION 3]: Accept pre-squared distances
...
data {
  ...
  matrix[N_spatial_bf, N_spatial_bf] dist_mat_anchors_sq; // NEW: Pre-squared from R
...
transformed parameters {
  ...
  vector[S] phi;
  {
    ...
    // [OPTIMIZATION 2]: Scalar sigma_phi moved to the outside
  ...
  vector[S] gamma;
  {
    ...
    // [OPTIMIZATION 2]: Scalar sigma_gamma moved to the outside
...
model {
...

  {
    ...
    {
       int start = 1; 
       // [OPTIMIZATION 1]: Inline vector subsetting directly from static_niche
    ...
    for (t in 2:T_total) {
      ...
      {
         int start = (t-1)*S + 1;
         // [OPTIMIZATION 1]: Inline vector subsetting, zero matrix overhead
